...
SELECT Count(p.`ID_Producte`) as Nombre_Productes_Venuts, b.`Localitat` FROM producte p INNER JOIN detallcomanda dc ON dc.`ID_Producte` = p.`ID_Producte` INNER JOIN comanda c ON c.`ID_Comanda` = dc.`ID_Comanda`INNER JOIN botiga b ON b.`ID_Botiga` = c.`ID_Botiga` WHERE p.`TipusProducte` = 'Beguda' AND b.`Localitat` = 'Barcelona' GROUP BY b.`Localitat`;
